...
Skills are defined as `SKILL.md` files in `skills/`. When you invoke a slash command, Claude Code loads the markdown and follows the instructions — no Python code is executed by the skill itself. All data operations go through the Distillery MCP server's tools.
