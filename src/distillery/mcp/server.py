...
def create_server(config: DistilleryConfig | None = None, auth: Any | None = None) -> FastMCP:
    ...
    async def distillery_correct(  # noqa: PLR0913
        ...
        The original entry is archived and the new correction entry links to it
        via metadata.related_entries. Fields (entry_type, author, project, tags)
        are inherited from the original when not provided.
        """
        c = _lc(ctx)
        user = _get_authenticated_user()
